...
import {
  ...
  CustomerDetails,
  Products,
} from 'src/app/entities/ClientsModel';
...
export class NewSalesInvoiceComponent implements OnInit, AfterViewInit {
  ...
  productSelection(product) {
    ...
    this.ProdMaterial = [];
    let material = this.products.filter((item) =>
      item.ProductCategory.toLowerCase().includes(
        this.selectProduct.toLowerCase()
      )
    );
    if (material) {
      this.ProdMaterial = Array.from(
        new Set(material.map((item) => item.ProductMaterial.toLowerCase()))
      );
      if (this.ProdMaterial && this.ProdMaterial.length == 1) {
        this.selectedMaterial = this.ProdMaterial[0];
        let size = this.products.filter(
          (item) =>
            item.ProductCategory.toLowerCase().includes(
              this.selectProduct.toLowerCase()
            ) &&
            item.ProductMaterial.toLowerCase().includes(
              this.selectedMaterial.toLowerCase()
            )
        );
        if (size) {
          this.ProdSize = Array.from(
            new Set(size.map((item) => item.ProductSize.toLowerCase()))
          );
  ...
  addItem() {
    ...
    if (this.isSizeError || this.isMaterialError) {
      ...
      obj[
        'orderid'
      ] = `${new Date().getHours()}${new Date().getMilliseconds()}`;
      this.prodtableData.push(obj);
      this.resetSelectedVales();
      this.qty = 0;
    }
  ...
  convertClass(data): any {
    ...
    data.forEach((obj) => {
      ...
      prod.price = (
        Number(obj['prodprice']) / Number(obj['prodqty'])
      ).toString();
      prod.category_id = '2';
  ...
  AddNewCustomer() {
    ...
    // this.service.CreateCustomer(this.newCustomer).subscribe((res) => {
  ...
  reduceMaterials() {
    this.prodtableData.forEach((item) => {
      let prod = this.filerProduct(
        item['prodname'],
        item['prodmaterial'],
        item['prodsize']
      );
